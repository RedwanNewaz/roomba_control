...
controller::controller(shared_ptr<state_estimator> localization, int control_fq):
localization_(localization)
{
    ...
    nh_.getParam("roomba_control/position_controller/kp", pos_kp);
    nh_.getParam("roomba_control/position_controller/kd", pos_kd);
    nh_.getParam("roomba_control/position_controller/ki", pos_ki);

    position_controller_ = new PID(update_time, -1, 1, pos_kp, pos_kd, pos_ki);

    double ori_kp, ori_kd, ori_ki;
    nh_.getParam("roomba_control/orientation_controller/kp", ori_kp);
    nh_.getParam("roomba_control/orientation_controller/kd", ori_kd);
    nh_.getParam("roomba_control/orientation_controller/ki", ori_ki);

    orientation_controller_ = new PID(update_time, -1, 1, ori_kp, ori_kd, ori_ki);

    nh_.getParam("roomba_control/goal_tolerance", threshold_);
...
void controller::publish_control_cmd(const ros::TimerEvent &event) {
    ...
    if(cntrl_state_ == RUNNING)
    {
        ...
        ROS_INFO_STREAM("x " << state[0] << " y " << state[1] << " theta " << state[2]);
...
void controller::callback_rviz_goal(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    ROS_INFO_STREAM("Recived RVIZ GOAL " << *msg);
